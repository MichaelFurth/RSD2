...
SDName: Ebon_Hold
SD%Complete: 85
SDComment: Quest support: 12848, 12733, 12739(and 12742 to 12750), 12727, 12698, 12720, 12754, 12698(HACK)
   ...
   void UpdateAI(const uint32 diff) 
   {
       if (uiSpeech_counter >= 1 && uiSpeech_counter <= 6)
           if (uiSpeech_timer < diff)
           {
               ...
               Unit* pPlayer = m_creature->GetMap()->GetCreature(uiPlayerGUID);
...
struct MANGOS_DLL_DECL mob_scarlet_minerAI : public ScriptedAI
{
    ...
    void SpellHit(Unit* pCaster, const SpellEntry* pSpell)
    {
        if (pCaster->GetTypeId() == TYPEID_PLAYER && m_creature->isAlive() && pSpell->Id == SPELL_GIFT_OF_THE_HARVESTER_MISSILE)
        {
            ...
            {
                if (rand()%100 < 35)
                    Unit* pGhoul = m_creature->SummonCreature(NPC_SCARLET_GHOUL, m_creature->GetPositionX(), m_creature->GetPositionY(), m_creature->GetPositionZ(), 0, TEMPSUMMON_CORPSE_DESPAWN, 1000);
                else
                    Unit* pGhost = m_creature->SummonCreature(NPC_SCARLET_GHOST, m_creature->GetPositionX(), m_creature->GetPositionY(), m_creature->GetPositionZ(), 0, TEMPSUMMON_CORPSE_DESPAWN, 1000);
...
struct MANGOS_DLL_DECL npc_scarlet_minecarAI : public ScriptedAI
{
    npc_scarlet_minecarAI(Creature* pCreature) : ScriptedAI(pCreature) 
    {
    ...
    void SummonedMovementInform(Creature* pSummoned, uint32 uiMotionType, uint32 uiPointId)
    {
        ...
        switch(uiPointId)
        {
            ...
            case 20:
            {
                ...
                pPlayer->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_OOC_NOT_ATTACKABLE);
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_uiMinerTimer < uiDiff)
        {
            ...
            m_Player = m_creature->GetCreatorGuid();
            Player* pPlayer = m_creature->GetMap()->GetPlayer(m_Player);
...
CreatureAI* GetAI_npc_scarlet_minecar(Creature* pCreature)
{
    return new npc_scarlet_minecarAI(pCreature);
}

enum
{
    QUEST_MASSACRE_POINT	= 12701,
    NPC_MINE_CAR			= 28817,
    SPELL_MOUNT				= 46598
};
...
{
    if (pPlayer->GetQuestStatus(QUEST_MASSACRE_POINT) == QUEST_STATUS_INCOMPLETE)
    {
        Unit* MineCar = pPlayer->SummonCreature(NPC_MINE_CAR,pPlayer->GetPositionX(),pPlayer->GetPositionY(),pPlayer->GetPositionZ(), 0 ,TEMPSUMMON_MANUAL_DESPAWN, 5000);
        MineCar->SetCreatorGuid(pPlayer->GetObjectGuid());
        MineCar->AddObjectToRemoveList();
        pPlayer->CastSpell(MineCar, SPELL_MOUNT, true);
    }
    return true;
}
...
void AddSC_ebon_hold()
{
    ...
    pNewScript = new Script;
    pNewScript->Name = "npc_scarlet_minecar";
    pNewScript->GetAI = &GetAI_npc_scarlet_minecar;
    pNewScript->RegisterSelf();

    pNewScript = new Script;
    pNewScript->Name = "go_Inconspicuous_Mine_Car";
    pNewScript->pGOHello =	&GOHello_go_Inconspicuous_Mine_Car;
    pNewScript->RegisterSelf();
}
